...
date: 2024-05-31T14:29:47
description: Adding Correlation to Logs in Isolated Azure Functions
---
# Adding Correlation to Logs in Isolated Azure Functions
...
1. **Azure Function App**: Ensure you have an Azure Function App deployed in your Azure subscription.

2. **Application Insights**: Make sure Application Insights is enabled for your Azure Function App.
...
2. Select **Application Insights** in the left-hand menu.

3. If not already configured, enable Application Insights and link it to your Function App.
    ...
    {        
	    ...
	    var host = new HostBuilder()  
            ...
            {  
                ...
                var configuration = hostContext.Configuration.GetSection("Logging");  
...
You will need to also configure the log configuration in appsettings.json
...
All the log lines will be automatically have Opereation Id as Correlation Id. You do not need to do anything to add the Correlation Id when call \_logger.Log. You can go directly to Step 5 without any problem.

In isolated Azure Functions, you can manually add correlation information to your logs using `TelemetryClient`. Here’s how you can do it:
...

{
    ...
    public MyFunction(TelemetryClient telemetryClient, ILogger<MyFunction> logger)
...

    {
        ...
        var operationId = req.FunctionContext.TraceContext.TraceParent.TraceId;

        var parentId = req.FunctionContext.TraceContext.TraceParent.ParentId;





        // Log a custom event with correlation
...
2. **Navigate to the **Transaction Search** or **End-to-End Transaction Details**:

   - You should see correlated telemetry items grouped by operation IDs.

3. **Use the **Application Map** to visualize the end-to-end flow of requests across services**.
...
- **Custom Telemetry**: For non-.NET languages or custom telemetry, you can manually add correlation IDs to your logs and telemetry items.

- **HTTP Headers**: Ensure that your services pass correlation headers (`Request-Id` and `Correlation-Context`) along with HTTP requests to maintain correlation across services.
